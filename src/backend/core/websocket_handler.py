...
class TranscriptionWebSocket:
    ...
    async def handle_transcription(self, websocket: WebSocket):
        ...
        try:
            ...
            while True:
                ...
                if "bytes" in data:
                    ...
                    await self._process_audio_chunk(
                        websocket, audio_chunk, finalize=False
                    )
            ...
            try:
                ...
                logger.error(
                    f"Failed to send websocket error message to client: {send_error}"
                )
            self.disconnect(websocket)

    async def _process_audio_chunk(
        self, websocket: WebSocket, audio_chunk: bytes, finalize: bool = False
    ):
        """
        ...
        try:
            ...
            async for text in self.whisper_model.transcribe_stream(
                audio_chunk, finalize=finalize
            ):
                if text.strip():
                    # Send transcribed text back to client
                    await websocket.send_json(
                        {"type": "transcription", "text": text, "is_final": finalize}
                    )
    ...
    async def _handle_command(self, websocket: WebSocket, message: dict):
        ...
        if command == "finalize":
            ...
            await websocket.send_json(
                {"type": "finalized", "message": "Transcription finalized"}
            )

        elif command == "reset":
            # Reset audio buffer
            self.whisper_model.reset_buffer()
            await websocket.send_json({"type": "reset", "message": "Buffer reset"})
    ...
    async def broadcast(self, message: dict):
        ...
        for connection in self.active_connections:
            try:
                ...
                logger.error(f"Failed to send message to a client: {send_error}")
