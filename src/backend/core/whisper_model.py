...
class WhisperModel:
    ...
    async def transcribe_stream(
        ...
        if audio_chunk:
            ...
            self.chunk_count += 1
            logger.debug(f"Accumulated chunk #{self.chunk_count}, buffer size: {len(self.audio_buffer)} bytes")
        ...
        if finalize and len(self.audio_buffer) > 0:
            try:
                logger.info(f"Finalizing transcription with {len(self.audio_buffer)} bytes from {self.chunk_count} chunks")
