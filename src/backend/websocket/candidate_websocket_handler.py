...
class TranscriptionWebSocket:
    ...
    def __init__(
        self,
        transcriber: StreamingFasterWhisperTranscriber,
        client: AsyncOpenAIClient,
        stt_cleaner_model: str,
    ):
        ...
        while True:
            ...
            if "bytes" in data:
                ...
                if message.get("command") == "reset":
                    ...
                    logger.info(
                        f"Final transcript requested (pre-cleanup): {final_text}"
                    )
                    ...
                    if final_text:
                        try:
                            (
                                cleaned_text,
                                usage,
                            ) = await self.client.clean_stt_transcript(
                                final_text, event_name="candidate_response_cleanup"
                            )
                        except Exception as e:
                    ...
                    logger.info(
                        f"Final transcript requested (after cleanup): {cleaned_text}"
                    )
                    await websocket.send_json(
                        {"type": "full_transcript", "text": cleaned_text}
                    )
        ...
        async for text in self.transcriber.stream_transcribe():
            if text == "[COMMIT]":
                ...
                if committed:
                    ...
                    logger.debug(
                        f"Full transcript so far: {self.full_transcript.strip()}"
                    )
                ...
                await websocket.send_json(
                    {"type": "transcription", "text": text, "is_final": False}
                )
