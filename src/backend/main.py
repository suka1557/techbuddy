...
config = Config.from_yaml(
    os.path.join(
        ...
        "backend",
        "configs",
        os.getenv("ENVIRONMENT", "dev") + ".yaml",
    )
)
...
if not os.getenv("OPENAI_API_KEY"):
    logger.error(
        "OPENAI_API_KEY is not set in environment variables. Please set it before running the application."
    )
...
model_dir = model_cfg.get(
    "model_dir", os.path.join(ROOT_PATH, "src", "backend", "models")
)
...
logger.info(
    f"Database configuration - Host: {DB_CONFIG['host']}, Port: {DB_CONFIG['port']}, Name: {DB_CONFIG['dbname']}, User: {DB_CONFIG['user']}"
)
...
question_provider = QuestionProvider(
    file_path=config.get("questions", {}).get("file_path")
)
...
async def startup_event():
    ...
    await minio_client.create_path(BUCKET_NAME, "resumes/")
